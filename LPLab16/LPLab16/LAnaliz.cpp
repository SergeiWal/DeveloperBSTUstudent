...
namespace LA
{
			...
			else
			{
				...
				case LA::L:
					...
					case IT::INT:
						...
						//if (words->Array[i-1].value[0] == MIN)itNewEntry->value.vint = -itNewEntry->value.vint;
						//if(itNewEntry->value.vint > (pow(2,31) + 1) || itNewEntry->value.vint < -pow(2, 31))throw ERROR_THROW_IN(128, words->Array[i].line, 0);
						break;
					default:
						break;
					}
				...
				case LA::V:
					...
					itNewEntry->idtype = IT::O;
					switch (words->Array[i].value[0])
					{
					case '+':
						itNewEntry->id[0] = '+';
						itNewEntry->id[1] = '\0';
	...
	void idTableOut(IT::IdTable it)
	{
		...
		{
			if (it.table[i].idtype != IT::L && it.table[i].idtype != IT::O)
			{
				if (i < 10)out << "00";
				if (i < 100 && i >= 10)out << "0";
				out << i << "\t" << it.table[i].id << "\t";
	...
	void ltTableOut(IT::IdTable it)
	{
		...
		for (int i = 0; i < it.size; ++i)
		{
			...
			else if (it.table[i].idtype == IT::O)
			{
				if (i < 10)out << "00";
				if (i < 100 && i >= 10)out << "0";
				out << i << "\t";

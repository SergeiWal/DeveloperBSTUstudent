...
namespace Log
{
	...
	{
		if (log.stream->is_open())(*log.stream) << "Ошибка" << error.id << ": " << error.message << "(строка:" << error.inext.line << " позиция" << error.inext.col << ")" << '\n';
		else std::cout << "Ошибка: " << error.id << ": " << error.message << "(строка:" << error.inext.line << " позиция: " << error.inext.col << ")" << '\n';
	}
	void Close(LOG log)
	{
		log.stream->close();

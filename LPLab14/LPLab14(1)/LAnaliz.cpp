...
namespace LA
{
	...
	wordArray* reading(unsigned char* text, int len)
	{
	...
	void LAnalyzis(wordArray* words, IT::IdTable& it, LT::LexTable& lt)
	{
		...
		for (int i =0; i<words->Array.size(); ++i)
		{
			...
			if (words->Array[i].value.size() == 1 && (words->Array[i].value[0] == LEX_SEMICOLON || words->Array[i].value[0] == LEX_BRACELET ||
				...
				words->Array[i].value[0] == LEX_RIGHTHESIS))
			{
			...
			else
			{
				...
				case LA::I:
					...
					for (int j = 0; j < words->Array[i].value.size() && j < ID_MAXSIZE; ++j)
					{
						...
						if ((j + 1) == words->Array[i].value.size() || (j + 1) == ID_MAXSIZE)itNewEntry->id[j + 1] = '\0';
					}

					ltNewEntry->idxTI = IT::IsId(it, itNewEntry->id);

					if (ltNewEntry->idxTI == TI_NULLIDX)
					{
						if (lt.table[lt.size - 1].lexema[0] == 'f')
						{
							itNewEntry->idtype = IT::F;
							if (words->Array[i - 2].value[0] == 'i')itNewEntry->iddatatype = IT::INT;
							else if (words->Array[i - 2].value[0] == 's')itNewEntry->iddatatype = IT::STR;
						}
						else
						{
							if (lt.table[lt.size - 2].lexema[0] == 'd')itNewEntry->idtype = IT::V;
							else if (lt.table[lt.size - 2].lexema[0] == '(' || lt.table[lt.size - 2].lexema[0] == ',')itNewEntry->idtype = IT::P;

							if (words->Array[i - 1].value[0] == 'i')itNewEntry->iddatatype = IT::INT;
	...
	void idTableOut(IT::IdTable it)
	{
		...
		if (!out.is_open())throw ERROR_THROW(123);
		out << "\t" << "NAME" << "\t" << "DATATYPE" << "\t" << "TYPE" << "\t\t\t" << "REFERENCE\n";
